...
- 「グラフ」「可視化」「チャート」等の語を使って、アプリ側の表示に言及しない。分析結果は文章で完結させる。
- 本文に「### グラフ（自動生成）」またはそれに類する見出し/段落を作らない（「グラフ」「チャート」「可視化」「自動生成」を含む見出しは全面禁止）。
- 見出しはテンプレの「## 1〜## 10」以外を書かない。特に「###」の追加は禁止。
- もし本文中に「グラフ」「チャート」「可視化」「自動生成」に関する見出し/段落を書いてしまった場合、その段落を削除してから最終出力する。
...
- もし上記の禁止語が本文に混入した場合、その行/段落を削除してから最終出力する（削除した結果、文章が不自然なら言い換えて整形する）。
...
- 本文にアプリ機能やUIの説明を書かない（例：グラフ自動生成/グラフ画面/ボタン操作/画面で確認 等は禁止）。
- 本文に「グラフ（自動生成）」等の見出し/段落を絶対に出さない（グラフ/チャート/可視化/自動生成に言及しない）
"""