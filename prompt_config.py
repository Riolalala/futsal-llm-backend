...
注意:
- system_prompt は f-string にしない（{ } が混ざると Python が変数評価して落ちるため）
"""

# --- 本文テンプレ（型） ---
report_template: str = """\
【概要】大会名/対戦/会場を1文で要約（チーム名で記述）
...
- 必ずチーム名（入力の home.name と away.name）で書く。
...
(1) まず <FACTS> ブロックに、events から抽出した事実を整理する（必ず出力）
  ...
  - 集計スコア（必須。チーム名で表記）
      * 前半: HOME_TEAM X - Y AWAY_TEAM
      * 後半: HOME_TEAM X - Y AWAY_TEAM
      * 合計: HOME_TEAM X - Y AWAY_TEAM
